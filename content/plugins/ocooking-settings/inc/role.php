...
class oCookingRole
{
    ...
    public function removeCap()
    {
        ...
        foreach ($array_roles as $role_name) {
            ...
            $role = get_role($role_name);
            $role->remove_cap('delete_recipes');
            $role->remove_cap('delete_private_recipes');
            $role->remove_cap('delete_published_recipes');
            $role->remove_cap('delete_others_recipes');
            $role->remove_cap('edit_private_recipes');
            $role->remove_cap('edit_published_recipes');
            $role->remove_cap('edit_others_recipes');
            $role->remove_cap('edit_recipes');
            $role->remove_cap('publish_recipes');
        }
    }
